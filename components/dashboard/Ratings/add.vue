<template>
  <div>
    ...
    <div class="flex flex-row-reverse mt-3">
      <button class="bg-[#4f46e5] hover:bg-[#4f46e5ab] focus:outline-none text-white p-2 rounded-xl" @click="addTheRatings()">Add Rating</button>
    </div>
  </div>
</template>
      ...
      this.$refs.addRatesRef.validate(async (valid) => {
        if(valid) {
          ...
          try {
            ...
            await this.$axios.post('/rates', formData);
