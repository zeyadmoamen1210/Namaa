<template>
  <div>
    <div class="flex items-center justify-between mb-5 flex-wrap gap-[10px]">
      <h5 class="text-2xl font-semibold mb-0">Certificates</h5>
      <div class="flex gap-3 items-center">
        ...
        <nuxt-link tag="button" class="bg-[#4f46e5] hover:bg-[#4f46e5ab] focus:outline-none text-white p-2 rounded-xl" to="/dashboard/certificates/add" >Add Certificate</nuxt-link>
      </div>
    </div>
    <div v-if="certificates.length > 0" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-[30px] ">
      <div v-for="item in certificates" :key="item._id">
        <CertificateCard :to="`/dashboard/certificates/${item._id}`" :editable="true" :item="item" @getCertificate="getCertificate" />
      </div>
    </div>
    <NoData v-else />
  </div>
